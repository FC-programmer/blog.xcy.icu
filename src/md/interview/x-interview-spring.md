...
假设我们只有一个缓存容器，缓存是直接开放给用户使用的，如果将未完成赋值的Bean和已完成赋值的Bean全部放在同一个容器中，那么调用者就可能拿到未完成赋值的Bean，此时当前的Bean是不可用的，会产生空指针异常。
